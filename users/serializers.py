...
class OwnedHyperlinkedModelSerializer(HyperlinkedModelSerializer):
    def validate(self, data):
        data = super(OwnedHyperlinkedModelSerializer, self).validate(data)
...
def get_owned_hyperlinked_related_field(primary_key_class, *attr_args, **attr_kwargs):
    class OwnedHyperlinkedRelatedField(HyperlinkedRelatedField):
        def get_queryset(self):
            current_user = self.context["request"].user
            queryset = primary_key_class.objects.filter(owner__id=current_user.id)
